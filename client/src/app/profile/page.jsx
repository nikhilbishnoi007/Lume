...
const Page = () => {
    ...
    const handleDeletePost = async (postId) => {
        setOpenMenuId(null)
        const result = await showConfirm("do you want to delete the post")
        if (!result) {
            return
        }
        try {
            const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/rest/delete/${postId}`, {
                method: "DELETE",
                credentials: "include"
            })
            const data = await res.json()
            if (data.success) {
                setusers(prev => ({ ...prev, post: prev.post.filter(p => p._id !== postId) }))
                showToast(data.message)
            }
            else{
                showToast(data.message)
                            ...
                            {users.post?.map((post) => (
                                <div key={post._id} className="bg-white rounded-lg shadow-md border border-gray-200 overflow-hidden flex flex-col">
                                    <div className="flex items-center gap-2 p-3 justify-between">
                                        ...
                                        <button  onClick={() => setOpenMenuId(openMenuId === post._id ? null : post._id)} className=" cursor-pointer  active:scale-95 transition-all duration-150 flex gap-1 items-center">
                                    ...
                                    <div className="relative w-full h-64 bg-zinc-100">
                                        {openMenuId === post._id && (
                                            <div className="absolute right-0 top-0 bg-white border border-gray-200 rounded-lg shadow-md w-32 z-10">
                                                <button   onClick={() => handleDeletePost(post._id)}  className="flex items-center gap-2 w-full px-3 py-2 text-sm text-red-600 hover:bg-zinc-50">
                                                    <Trash2 size={14} />Delete
                                                </button>
                                            </div>
                                        )}
                                        <Image src={post.image} alt="post image" fill className="object-cover"onClick={() => setOpenMenuId( null )}  />
                                    </div>
